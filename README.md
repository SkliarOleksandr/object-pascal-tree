...
1. **Find the rule.** If the spec already states it, the fix follows its wording.
   ...
   patching symptoms - sec. 5.7 says a `with` target is a structured-typed
...
3. **If the spec and the code disagree, do not assume the spec is right.** It is
   evidence, not scripture: sec. 1.2.2 once cited `Generics.Collections` as the
...
4. **Check before you write.** sec. 13.1.4 already described default array
...
(`&Foo` and `Foo` being the same identifier, sec. B.3) would have gone straight to
...
| Path | Contents |
|---|---|
| `source/` | the library: `PasTree.Types`, `PasTree.SourceManager`, `PasTree.Lexer`, `PasTree.Preprocessor`, `PasTree.Ast`, `PasTree.Parser`, `PasTree.DProj`, `PasTree.Platforms`, `PasTree.Ast.Json`, `PasTree.Ast.Check` (the tree checker: span, link and Aux invariants over any tree, and the own-token walk), `PasTree.Project`, and the semantic layer `PasTree.Sema.*` (`Model`, `Resolver`, `Types`, `Project`, `Builtins`, `Nav`, `Async`, `Diagnostics`, `Dump`) |
...
| `tools/` | CLI drivers per pipeline stage (`PasTreeLex`, `PasTreePP`, `PasTreeParse`, `PasTreeJson`, `PasTreeSema`, `PasTreeSemaProject`), `PasTreeTreeCheck` (the tree checker over a directory or the golden rows: every invariant violation, and a per-kind histogram of the tokens each node kind owns), `PasTreeDcu` (a `.dcu` -> interface source, a raw dump, or the read-and-parse sweep over a library directory - the `.dcu` reader's regression gate), the node-kinds generator, and the compile-compare harness under construction: `PasTreeXform` rewrites a unit along its tree, `fidelity.ps1` compiles original and rewrite under dcc and compares the `.dcu` bytes (identical exactly when the tree agrees with dcc's), naming the differing routine from the dump |
...
- ~~**An interface's implicit `IInterface` ancestor is not walked.**~~ **Done
  ...
  that it already made for a heritage-less class (14 sec. 14.1.1: the default
...
- ~~**A third-party collections/DI library is a corpus now.**~~ **Clean since
  ...
  became a type error. **Arity is part of a type's identity** (sec. 16.1.2), and the
  ...
  sec. 16.1.2 did not mention and now does. The guards are tested inline in that
  ...
  other.HasValue` became `not <string>`. sec. 16.1.2 states both directions and the
...
- ~~**Unresolved MEMBERS after a dot are reported only behind a FLAG.**~~
  ...
  **Every RTL/VCL/FMX corpus reports NOTHING under the member flag as of
  ...
  6 sec. 6.6.1 - which stated the syntax and not this). The hop carries the
  ...
  **The server project, 2026-08-08: 56 -> ZERO.** Nine causes over two passes. The
  ...
  - **several constraints on one parameter** (2). `TKey: IComparable<TKey>,
    ...
    (16 sec. 16.4.1); the hop took the FIRST, and a utility library calls `AKey
  ...
  **The third-party library, 2026-08-08: base 19 -> 0, core 27 -> 2, data
  ...
  - **the re-export idiom, `X = X`** (2 sec. 2.5.1, written up from this): the right
  ...
  - **a builtin type has several NAMES, and a helper targets the type.**
    ...
    at most one helper is active per type (15 sec. 15.3.3) - so it HID
  ...
  - **a generic METHOD's constraints are on its own declaration** (16 sec. 16.2.1),
  ...
  - ~~**`_AddRef` / `_Release` / `QueryInterface`**~~ - **fixed 2026-08-06, and
    ...
    generic PARAMETER and stopped, having no rule for one. 16 sec. 16.4.1 says what
  ...
  - ~~**`Create` and `Free`, the tail after those three.**~~ **Fixed 2026-08-06 -
    ...
    - **`T: class` guarantees TObject's members.** sec. 16.4.1 listed the constraint
...
- ~~**Inline `var`/`const` visibility is not POSITIONAL.**~~ **Done 2026-07-31**
  ...
  corrected here on 2026-08-07, the second one in this list to do that. 3 sec. 3.1.3
...
- ~~**Member visibility is recorded but not ENFORCED.**~~ **Both halves are done
  ...
  **Then the tail, worked one site at a time on 2026-08-07: 546 -> 2 on rtlflat
  ...
  - **`strict private` reaches a NESTED type** (the `FJSONWriter`/`FParentTypes`
    ...
    one's - that direction is dcc's own `E2361`. Now in 11 sec. 11.2.1.
  - **a `class constructor` is never what a name means** (15 sec. 15.1.5 said "not
  ...
  **The last twelve, read end to end the same day: 6 -> 2 on rtlflat and 12 -> 2
  ...
  (2 sec. 2.2.4) and a `strict private` nested helper still activates (15 sec. 15.4).
...
- **Missing semantic CHECKS the spec names, none of which can produce a false
  ...
  - ~~no "ordinal type required" check anywhere~~ - **done 2026-08-05**
    ...
    positions, so the code was right to accept it, and sec. 2.1.1/sec. 2.2.4 have been
    ...
    which is now a table in sec. 2.1.1. The `case` selector followed the same day,
  ...
  - ~~bare `raise` outside an exception handler~~ - **done 2026-08-05**
    ...
    does not - all dcc-verified, and now written into sec. 18.3.1, which had said
    only "the analyzer must track handler context". Zero new diagnostics across
    all four corpora;
  - ~~`Slice` outside an open-array argument position (4 sec. 4.11)~~ - **done
    2026-08-05** (`E2193`), and dcc turned out to be stricter than sec. 4.11 said:
    ...
    argument;
  - ~~multiline-string indentation (B sec. B.6.3)~~ - **done 2026-08-05**
    ...
    sec. B.6.3 said "less indented", which predicts neither of those; it now has the
...
- ~~**Three rules the CODE implements that the SPEC never states.**~~ **Written
  ...
  (sec. 15.3.1), the dynamic `array of array of T` indexing sugar (sec. 8.2.1), and a
  `TypeRef` position resolving to a TYPE over a nearer same-named non-type
  (sec. B.11). Behaviour unchanged - the code already did all three.
- **Where the audit stands (2026-08-05).** ALL FIFTEEN of its items are closed -
  ...
  - ~~a seeded builtin name beating a class's own member~~ - **done 2026-08-06,
    ...
    it wins in a TYPE position too, both probed and now in 11 sec. 11.4). We bound
...
- **Audit coverage, so the next pass knows where to start.** The 2026-07-31
  ...
  `PasTree.Sema.Types` in particular has never been read against ch.02 sec. 2.6.1,
...
- **Two over-permissive bindings, both found on 2026-08-08 and both left in
  ...
  - *A lone `constructor` constraint unlocks all of TObject, where dcc unlocks
    only `T.Create`.* 16 sec. 16.4.1 is explicit and probed: under `T: constructor`
  ...
  - *An inherited overload set is merged whether or not the declarations say
    ...
    directive, so it accepts the hidden call too. Now in the spec (6 sec. 6.3.1).
...
- ~~**Rename symbol** (Ctrl+Shift+E)~~ - DONE (`TPasNavigator.PlanRename` /
  ...
  identity Find References walks, so it only touches the actual
  declaration's references and never a textually matching name from an
  unrelated scope. Also renames a UNIT (its header + every `uses` item,
  dotted names whole; the required file name is handed back to the host).
  A compiler builtin is refused outright - the name is the compiler's - and
  so is a rename that would touch a LIBRARY source (RTL/VCL/third-party: the
